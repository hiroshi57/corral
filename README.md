...
## 🤔 どんな時に使うのか（まずこれを読んでください）

**一言でいうと「1人で順番にやると待ち時間が長い作業を、並べて回して待たずに済ませる」ためのツールです。**

| 今のやり方 | Corral |
|---|---|
| 指示 → 終わるまで待つ → 次の指示 …（タブが増えて管理不能） | 5件まとめて出して**並列実行**。終わったものが「要確認」に上がる |
| どのタブが何をしていたか分からない | 1画面で状態を一覧。完了は **Chatwork/Slack に通知** |
| 変更が作業中のコードに混ざる | 各タスクが**専用の作業コピー（git worktree）**で作業。本体は汚れない |
| 何をどう直したか後から追えない | 差分レビュー・承認・**監査ログ**が残る |

**使いどころ**：提案書からタスクを起こして一気に片付ける／複数リポに同じ改修を横断投入／定型手順を型（プレイブック）にして回す／夜間に走らせて朝に確認。

**プログラムの案件だけでなく、資料の案件でも使えます。** 提案書・議事録・レポートが入った
フォルダを `CORRAL_REPO` に指定すれば、そのまま案件になります（変更履歴の管理は初回に自動で用意。
元ファイルは承認するまで変更されません）。→ [コードでない案件の使い方](docs/usage-scenarios.md#コードでない案件提案書議事録レポートの使い方)

**使わないほうがよい場面**：作業が1つだけ／対話しながら設計を詰めたい／1〜2分で終わる修正。→ その場合は普通に Claude Code を1つ使うほうが速いです。

👉 具体的な使い方の流れは **[docs/usage-scenarios.md](docs/usage-scenarios.md)**（実務シナリオ4本＋最初の5分の手順）
